...
最新发布说明：[`changelogs/v1.5.0.md`](./changelogs/v1.5.0.md)
...
- **文件编辑器支持图片和 PDF 预览**
  - 内置编辑器现在可以直接打开图片（`png`、`jpg`、`gif`、`webp`、`bmp`、`ico`、`svg`、`avif`）和 PDF——支持翻页、放大缩小和"第 X / Y 页"指示
  - 编辑器面板标题从"文本编辑器"更新为 **文件编辑器**，反映它不再只能处理文本
- **粘贴时新增"保留两者"选项**
  - 粘贴冲突对话框新增 **保留两者**：冲突项会以自动编号后缀的形式粘入（例如 `report.pdf` → `report (2).pdf`），原文件保持不变
  - 通过端到端的 `conflictStrategy`（`error` / `overwrite` / `rename`），本地与 SSH 文件传输共享一致的冲突处理语义
- **文件行右键菜单新增"复制完整路径"**
  - 右键文件或文件夹即可把绝对路径（多选时为多条路径）写入系统剪贴板，可直接粘贴到终端、对话或编辑器
- **SSH SFTP 写入更稳**
  - 分块写入接口在最后一个分块及空文件写入时会主动关闭 SFTP 句柄，不再依赖会话 GC 才释放
- **Seamless 模式工具组明显可展开**
  - 工具组的箭头默认就有可见度（不再只有悬停时显示），单步骤组也使用与多步骤组相同的可展开 header，可以一眼看出"这一组可以点开"
  - 展开工具组后会显示每一步**完整、未截断**的调用信息——完整命令、完整工具参数、完整文件路径。工具的*输出内容*仍然不会展示
  - `N steps` 步骤计数与标题之间也增加了合适的间距
- **紧凑横向监控：柱状条高度修复**
  - 紧凑横向监控布局下的 DISK / LOAD / RX 柱状条过去会塌缩成最低高度的细条（而 CPU / MEM / TX 正常显示），现在所有柱状条都按真实百分比稳定渲染；0% 时刻意不显示填充块，让"没有信号"和"信号很弱"在视觉上能够明确区分
