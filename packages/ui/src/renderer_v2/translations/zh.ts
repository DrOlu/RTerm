export const zh = {
  ...
  settings: {
    ...
    accessTokenCreatedHint: '明文令牌只会显示一次。若遗失，只能删除该令牌并重新创建。',
    accessTokenIssuedList: '已发放令牌',
    noAccessTokens: '暂无访问令牌',
    confirmDeleteAccessToken: (name: string) => `确定删除访问令牌「${name}」吗？使用该令牌的客户端将在下次重连时失效。`,
    checkVersion: '检查更新',
    checkingVersion: '检查中',
    currentVersion: '当前版本',
    latestVersion: '最新版本',
    ...
    tooltips: {
      ...
      commandDraftShortcut: '点击右侧快捷键字段后开始录制，用来设置在 terminal tab 中打开命令草稿悬浮窗的前端快捷键。录制时按 Esc 会直接禁用这个快捷键。',
      ...
      compactionModel: '仅用于历史压缩总结。只要这个 Profile 里任意一个已配置模型不支持图片，整个 Profile 下所有模型调用都会收到去图片后的历史。建议 Compaction 模型的 maxTokens 大于等于 Global 模型。',
      ...
      memory: '记忆是全局唯一的 markdown 文件（memory.md），跨会话共享。启用后，后端会把该文件完整内容连同绝对路径和 create_or_edit/read_file 使用说明注入到会话 system prompt。',
      accessTokens: '配置 websocket 网关暴露范围与端口，并管理远程访问令牌。令牌只在创建时显示一次明文，磁盘上仅保存加盐哈希。',
    },
    skillSections: {
      custom: 'GyShell 自定义',
      others: '外部技能',
  ...
  filesystem: {
    ...
    confirmDeleteMany: (count: number) => `确定删除选中的 ${count} 项吗？该操作不可撤销。`,
    ...
    clipboardReadyToPasteCopy: (count: number) => `剪贴板：复制 ${count} 项。切换到目标 tab 后按 Cmd/Ctrl+V 粘贴。`,
    clipboardReadyToPasteMove: (count: number) => `剪贴板：剪切 ${count} 项。切换到目标 tab 后按 Cmd/Ctrl+V 粘贴。`,
    ...
    previewTooLarge: (name: string, maxMb: number) => `文件过大，暂不支持预览（${name}，限制 ${maxMb}MB）。`
  },
  fileEditor: {
    title: '文本编辑器',
