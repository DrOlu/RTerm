...
export const ChatHistoryPanel: React.FC<ChatHistoryPanelProps> = observer(
  ({ store, onClose }) => {
    ...
    const handleConfirmRename = async (
      ...
      try {
        // Optimistic update
        setHistory((prev) =>
          prev.map((s) => (s.id === editingId ? { ...s, title: newTitle } : s)),
        );
    ...
    const handleBulkDelete = async () => {
      ...
      try {
        ...
        await store.chat.deleteChatSessions(idsToDelete);
    ...
    const allVisibleSelected =
      ...
      filteredHistory.every((session) => selectedIds.has(session.id));
            ...
            >
              <div className="chat-history-item-main">
                {editingId === session.id ? (
                  <div
                    className="chat-history-item-edit-wrapper"
                ...
                <div className="chat-history-item-meta">
                  ...
                  <span className="chat-history-item-messages">
                    {session.messagesCount} {t.chat.history.messages}
                  </span>
                </div>
              </div>
              <button
    ...
    return createPortal(
      ...
      >
        <ConfirmDialog
          ...
          onConfirm={() =>
            confirmDeleteId && handleDeleteSession(confirmDeleteId)
          }
          onCancel={() => {
            if (isDeletingSingle) return;
            setConfirmDeleteId(null);
                ...
                {isSelectionMode && (
                  <>
                    ...
                    >
                      <Trash2 size={14} />
                      <span>
                        {t.chat.history.deleteSelected} ({selectedIds.size})
                      </span>
                    </button>
                  </>
                )}
